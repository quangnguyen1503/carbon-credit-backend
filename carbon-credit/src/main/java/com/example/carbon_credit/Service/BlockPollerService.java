...
public class BlockPollerService {
    ...
    public void init() {
        try {
            ...
            log.info("📍 Listened contracts: {}", chainConstants.getListenedAddresses()); // ✅ Use getter
    ...
    public void pollBlocks() {
        try {
            ...
            if (fromBlock.compareTo(safeBlock) > 0) {
                log.debug("⏸️ Already synced. Last: {}, Current: {}, Safe: {}", 
    ...
    private void scanAndProcess(BigInteger from, BigInteger to) throws IOException {
        try {
            ...
            EthFilter filter = new EthFilter(
                ...
                listenedAddresses // ✅ Use from bean
            );
            ...
            if (logs == null) {
                log.warn("⚠️ RPC returned null logs for blocks {} -> {}", from, to);
                return;
            }
            ...
            for (EthLog.LogResult logResult : logs) {
                try {
                    if (logResult == null) {
                        log.warn("⚠️ Skipping null log result");
                        continue;
                    }
                    ...
                    if (logData == null) {
                        log.warn("⚠️ Skipping null log data");
                        continue;
                    }
    ...
    private void processSingleLog(Log logData) {
        try {
            if (logData.getTopics() == null || logData.getTopics().isEmpty()) {
                log.warn("⚠️ Log has no topics, skipping");
                return;
            }
            ...
            if (eventType == null) {
                log.debug("⚠️ Unknown event hash: {}", eventHash);
                return;
            }
